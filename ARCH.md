...
- **Session addressing:** every operation that *writes* to a heading keys on the UUID, never the label. TODO.md is a file the user edits by hand, so two headings can share a label at any instant — paste a second `## New Session` block and nothing has run in between — and every label lookup takes the *first* match, which would silently redirect a send or a scheduled delivery onto the wrong session. The one heading a UUID cannot address is one an older jc left unbound (empty `> uuid=`), since the empty string names every such heading at once; that case is addressed by *verified position* — see the `todo::SessionKey` doc comment in `jc-core/src/todo.rs`, which is the canonical statement and which other comments point at. jc used to keep labels unique instead (a `unique_label` at every creation point plus a startup `dedupe_labels` repair); that guarantee held only until the user's next paste, and is gone.
