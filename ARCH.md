...
The second is the **launch-settle window**, which lives in `jc-terminal` next to the events that drive it. A freshly spawned `claude` prints a banner and, when resuming, replays its transcript; that is jc launching the session, not work done while you were away. `SessionState::create` calls `TerminalView::discount_launch_output` on the Claude terminal just after spawning it, which opens a `SettleWindow`; the notification relay drives it. That relay already blocks on the batch signal to schedule repaints, so it is woken by exactly the event the window needs, and there is no second task and no poll: each pass asks the window what to do, then waits for either the next batch or the window's own deadline, whichever comes first (`recv_or_deadline`). A batch restarts the quiet wait; a deadline means the window is due to decide. The policy is `settle.rs`'s two `Duration`s: `LAUNCH_QUIET` (4 s of quiet after the child has printed *something*) and `LAUNCH_GIVE_UP` (30 s absolute). `SettleWindow` is pure — it observes `Instant`s and answers `Clear`/`Wait`/`Done` — so the policy is unit-tested without a PTY.
