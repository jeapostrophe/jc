...
impl From<Keystroke> for Kbd {
  fn from(stroke: Keystroke) -> Self {
    Self { style: StyleRefinement::default(), stroke, appearance: true }
  }
}
      ...
      _ => {
        if key_str.len() == 1 {
          ...
          if let Some(first_char) = chars.next() {
            keys.push_str(&format!("{}{}", first_char.to_uppercase(), chars.collect::<String>()));
          } else {
...
mod tests {
  ...
  fn test_format() {
    ...
    if cfg!(target_os = "macos") {
      ...
      assert_eq!(Kbd::format(&Keystroke::parse("ctrl-alt-shift-a").unwrap()), "Ctrl+Alt+Shift+A");
      assert_eq!(
