  ...
  Y: Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
  fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
    ...
    // Y scale
    let all_values: Vec<Y> =
      self.data.iter().flat_map(|d| vec![high_fn(d), low_fn(d), open_fn(d), close_fn(d)]).collect();
    ...
    PlotAxis::new().x(height).x_label(x_label).stroke(cx.theme().border).paint(&bounds, window, cx);
    ...
    for d in &self.data {
      ...
      let color: Hsla = if is_bullish { cx.theme().bullish } else { cx.theme().bearish };
      ...
      let (top, bottom) = if is_bullish { (close_y, open_y) } else { (open_y, close_y) };
