...
use gpui::{
  AnyElement, App, ClickEvent, ElementId, InteractiveElement as _, IntoElement, ParentElement as _,
...
impl SidebarMenu {
  ...
  pub fn new() -> Self {
    Self { style: StyleRefinement::default(), items: Vec::new(), collapsed: false }
  }
...
impl RenderOnce for SidebarMenu {
  fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
    v_flex().gap_2().refine_style(&self.style).children(
      self.items.into_iter().enumerate().map(|(ix, item)| item.id(ix).collapsed(self.collapsed)),
    )
  }
}
      ...
      .when(is_open, |this| {
        this.child(
          v_flex()
            ...
            .pl_2p5()
            .py_0p5()
            .children(self.children.into_iter().enumerate().map(|(ix, item)| item.id(ix))),
        )
      })
  }
}
