...
use gpui::{
  ...
  ParentElement, Pixels, RenderOnce, StyleRefinement, Styled, Window, div, prelude::FluentBuilder,
  px,
};
use std::rc::Rc;
...
impl SidebarToggleButton {
  fn new(side: Side) -> Self {
    Self { btn: Button::new("collapse").ghost().small(), collapsed: false, side, on_click: None }
  }
...
impl RenderOnce for SidebarToggleButton {
  fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
    ...
    let icon = if collapsed {
      if self.side.is_left() { IconName::PanelLeftOpen } else { IconName::PanelRightOpen }
    } else {
      if self.side.is_left() { IconName::PanelLeftClose } else { IconName::PanelRightClose }
    };
          ...
          v_flex()
            ...
            .children(
              self
                ...
                .into_iter()
                .enumerate()
                .map(|(ix, c)| div().id(ix).mt_3().child(c.collapsed(self.collapsed))),
            )
