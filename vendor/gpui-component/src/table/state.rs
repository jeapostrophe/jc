  ...
  D: TableDelegate,
{
  ...
  pub fn set_selected_row(&mut self, row_ix: usize, cx: &mut Context<Self>) {
    ...
    if let Some(row_ix) = self.selected_row {
      self
        ...
        .scroll_to_item(row_ix, if is_down { ScrollStrategy::Bottom } else { ScrollStrategy::Top });
    }
  ...
  fn prepare_col_groups(&mut self, cx: &mut Context<Self>) {
    self.col_groups = (0..self.delegate.columns_count(cx))
      .map(|col_ix| {
        ...
        ColGroup { width: column.width, bounds: Bounds::default(), column: column.clone() }
      })
      .collect();
    cx.notify();
  }
  ...
  fn render_cell(&self, col_ix: usize, _window: &mut Window, _cx: &mut Context<Self>) -> Div {
    ...
    div()
      ...
      .map(|this| match col_padding {
        Some(padding) => this.pl(padding.left).pr(padding.right).pt(padding.top).pb(padding.bottom),
        None => this,
      })
  }
  ...
  fn render_col_wrap(&self, col_ix: usize, _: &mut Window, cx: &mut Context<Self>) -> Div {
    ...
    let selectable = self.col_selectable
      && self.col_groups.get(col_ix).map(|col_group| col_group.column.selectable).unwrap_or(false);
  ...
  fn render_resize_handle(
    ...
    let resizable =
      self.col_resizable && self.col_groups.get(ix).map(|col| col.is_resizable()).unwrap_or(false);
    if !resizable {
      ...
      .on_drag_move(cx.listener(move |view, e: &DragMoveEvent<ResizeColumn>, window, cx| {
        match e.drag(cx) {
          ResizeColumn((entity_id, ix)) => {
            ...
            let col_group = view.col_groups.get(ix).expect("BUG: invalid col index").clone();
          ...
          .child(
            h_flex()
              ...
              .when_some(paddings, |this, paddings| {
                ...
                let offset_pr = self.options.size.table_cell_padding().right - paddings.right;
          ...
          .when(movable, |this| {
            this
              ...
              .drag_over::<DragColumn>(|this, _, _, cx| {
                this.rounded_l_none().border_l_2().border_r_0().border_color(cx.theme().drag_border)
              })
      ...
      .when(left_columns_count > 0, |this| {
        ...
        this.child(
          h_flex()
            ...
            .child(
              canvas(
                move |bounds, _, cx| view.update(cx, |r, _| r.fixed_head_cols_bounds = bounds),
                |_, _, _, _| {},
              )
              .absolute()
              .size_full(),
      ...
      tr.h_flex()
        ...
        .child(
          h_flex()
            ...
            .relative()
            .child(
              crate::virtual_list::virtual_list(view, row_ix, Axis::Horizontal, col_sizes, {
                move |table, visible_range: Range<usize>, window, cx| {
                  ...
                  visible_range.for_each(|col_ix| {
                    ...
                    let el = table.render_col_wrap(col_ix, window, cx).child(
                      table
                        .render_cell(col_ix, window, cx)
        ...
        .when_some(self.selected_row, |this, _| {
          this.when(is_selected && self.selection_state == SelectionState::Row, |this| {
  ...
  fn measure(&mut self, _window: &mut Window, _cx: &mut Context<Self>) {
    ...
    if self._measure.len() > 0 {
      ...
      eprintln!("last render {} cells total: {:?}, avg: {:?}", self._measure.len(), total, avg,);
    }
    self._measure.clear();
  }
      ...
      .context_menu({
        ...
        move |this, window: &mut Window, cx: &mut Context<PopupMenu>| {
          if let Some(row_ix) = view.read(cx).right_clicked_row {
            view.update(cx, |menu, cx| menu.delegate_mut().context_menu(row_ix, this, window, cx))
          } else {
            this
          }
        }
