...
impl LoadingRow {
  fn header() -> Self {
    Self { header: true, size: Size::Medium }
  }

  fn row() -> Self {
    Self { header: false, size: Size::Medium }
  }
    ...
    h_flex()
      ...
      .child(
        h_flex()
          .gap_3()
          .flex_1()
          .child(Skeleton::new().when(self.header, |this| this.secondary()).h(height).w_24())
          .child(Skeleton::new().when(self.header, |this| this.secondary()).h(height).w_48())
          .child(Skeleton::new().when(self.header, |this| this.secondary()).h(height).w_16()),
      )
      .child(Skeleton::new().when(self.header, |this| this.secondary()).h(height).w_24())
  }
}
