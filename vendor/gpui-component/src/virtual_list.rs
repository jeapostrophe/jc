...
impl VirtualListScrollHandle {
  ...
  fn scroll_to_item_with_offset(&self, ix: usize, strategy: ScrollStrategy, offset: usize) {
    ...
    state.deferred_scroll_to_item =
      Some(DeferredScrollToItem { item_index: ix, strategy, offset, scroll_strict: false });
  }
  ...
  V: Render,
{
  ...
  VirtualList {
    id: id.clone(),
    axis,
    base: div().id(id).size_full().overflow_scroll().track_scroll(&scroll_handle),
    scroll_handle,
...
pub struct VirtualList {
  ...
  item_sizes: Rc<Vec<Size<Pixels>>>,
  render_items:
    Box<dyn for<'a> Fn(Range<usize>, &'a mut Window, &'a mut App) -> SmallVec<[AnyElement; 64]>>,
...
impl VirtualList {
  ...
  fn scroll_to_deferred_item(
    ...
    let Some(bounds) = items_bounds.get(scroll_to_item.item_index + scroll_to_item.offset).cloned()
    else {
...
impl Element for VirtualList {
  ...
  fn request_layout(
      ...
      |style, window, cx| {
        size_layout = window.with_element_state(
          ...
          |state: Option<ItemSizeLayout>, _window| {
            ...
            let gap = style.gap.along(self.axis).to_pixels(font_size.into(), rem_size);
            ...
            if state.items_sizes != self.item_sizes {
              ...
              state.sizes = self
                ...
                .map(|(i, size)| {
                  ...
                  if i + 1 == self.items_count { size } else { size + gap }
                })
                .collect::<Vec<_>>();
              ...
              window.request_measured_layout(style, {
                move |known_dimensions, available_space, _, _| {
                  ...
                  if axis.is_horizontal() {
                    size.width = known_dimensions.width.unwrap_or(match available_space.width {
                      AvailableSpace::Definite(x) => x,
                      AvailableSpace::MinContent | AvailableSpace::MaxContent => {
                        size_layout.content_size.width
                      }
                    });
                    size.height = known_dimensions.width.unwrap_or(match available_space.height {
                      AvailableSpace::Definite(x) => x,
                      AvailableSpace::MinContent | AvailableSpace::MaxContent => {
                    ...
                    size.height = known_dimensions.height.unwrap_or(match available_space.height {
                      AvailableSpace::Definite(x) => x,
                      AvailableSpace::MinContent | AvailableSpace::MaxContent => {
                        size_layout.content_size.height
                      }
    ...
    (layout_id, VirtualListFrameState { items: SmallVec::new(), size_layout })
  }
  ...
  fn prepaint(
    ...
    let paddings = style.padding.to_pixels(bounds.size.into(), window.rem_size());

    let item_sizes = &layout.size_layout.sizes;
    let item_origins = &layout.size_layout.origins;

    let content_bounds = Bounds::from_corners(
      bounds.origin + point(border_widths.left + paddings.left, border_widths.top + paddings.top),
      bounds.bottom_right()
        - point(border_widths.right + paddings.right, border_widths.bottom + paddings.bottom),
    );
    ...
    if let Some(scroll_to_item) = scroll_state.deferred_scroll_to_item.take() {
      scroll_offset =
        self.scroll_to_deferred_item(scroll_offset, &items_bounds, &content_bounds, scroll_to_item);
    }
