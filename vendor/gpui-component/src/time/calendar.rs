    ...
    match self {
      Self::Range(start, end) => {
        if let Some(start) = start {
          if let Some(end) = end { v >= *start && v <= *end } else { false }
        } else {
          false
        }
      }
...
impl CalendarState {
  ...
  pub fn set_date(&mut self, date: impl Into<Date>, _: &mut Window, cx: &mut Context<Self>) {
    ...
    let invalid = self.disabled_matcher.as_ref().map_or(false, |matcher| matcher.is_match(&date));
  ...
  pub fn year_range(mut self, range: (i32, i32)) -> Self {
    self.years = (range.0..range.1)
      ...
      .collect::<Vec<_>>();
    self.year_page =
      self.years.iter().position(|years| years.contains(&self.current_year)).unwrap_or(0) as i32;
    self
  }
  ...
  fn prev_month(&mut self, _: &ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
    ...
    self.current_year =
      if self.current_month == 12 { self.current_year - 1 } else { self.current_year };
    cx.notify()
  }

  fn next_month(&mut self, _: &ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
    self.current_month = if self.current_month == 12 { 1 } else { self.current_month + 1 };
    self.current_year =
      if self.current_month == 1 { self.current_year + 1 } else { self.current_year };
    cx.notify()
  }
...
impl Calendar {
  ...
  fn render_day(
    ...
    let disabled =
      state.disabled_matcher.as_ref().map_or(false, |disabled| disabled.matched(&date));
    ...
    self
      ...
      .when(is_today && !is_active, |this| this.border_1().border_color(cx.theme().border)) // Add border for today
      .when(!disabled, |this| {
        this.on_click(window.listener_for(&self.state, move |view, _: &ClickEvent, window, cx| {
  ...
  fn render_header(&self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    h_flex()
          ...
          h_flex()
            ...
            .child(
              Button::new("month")
                ...
                .on_click(window.listener_for(&self.state, move |view, _, window, cx| {
      ...
      .when(multiple_months, |this| {
        this.child(h_flex().flex_1().justify_around().children((0..self.number_of_months).map(
          |n| {
            h_flex()
              .justify_center()
  ...
  fn item_button(
    ...
    h_flex()
      ...
      .when(secondary_active, |this| {
        this
          .bg(if muted { cx.theme().accent.opacity(0.5) } else { cx.theme().accent })
          .text_color(cx.theme().accent_foreground)
      })
      .when(!active && !disabled, |this| {
        this.hover(|this| this.bg(cx.theme().accent).text_color(cx.theme().accent_foreground))
      })
      .when(active, |this| this.bg(cx.theme().primary).text_color(cx.theme().primary_foreground))
      .child(label.into())
  }
  ...
  fn render_days(&self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    h_flex()
      ...
      .justify_between()
      .children(state.days().chunks(5).enumerate().map(|(offset_month, days)| {
        v_flex()
          .gap_0p5()
          .child(
            h_flex()
              .gap_0p5()
              .justify_between()
              .children(weeks.iter().map(|week| self.render_week(week.clone(), window, cx))),
          )
          .children(days.iter().map(|week| {
            h_flex()
              .gap_0p5()
              .justify_between()
              .children(week.iter().map(|d| self.render_day(d, offset_month, window, cx)))
          }))
      }))
  }
      ...
      .children(
        months
          ...
          .map(|(ix, month)| {
            ...
            self
              .item_button(ix, month.to_string(), active, false, false, false, window, cx)
              .w(relative(0.3))
              .text_sm()
              .on_click(window.listener_for(&self.state, move |view, _, window, cx| {
      ...
      .children(
        current_page_years
          ...
          .map(|(ix, year)| {
            ...
            self
              .item_button(ix, year.to_string(), active, false, false, false, window, cx)
              .w(relative(0.2))
              .on_click(window.listener_for(&self.state, move |view, _, window, cx| {
