...
pub(crate) fn get_value<T: Clone + 'static>(field: &Rc<dyn AnySettingField>, cx: &mut App) -> T {
  let setting_field =
    field.as_any().downcast_ref::<SettingField<T>>().expect("Failed to downcast setting field");
...
pub enum SettingFieldType {
  Switch,
  Checkbox,
  NumberInput { options: NumberFieldOptions },
  Input,
  Dropdown { options: Vec<(SharedString, SharedString)> },
  Element { element: Rc<dyn SettingFieldElement<Element = AnyElement>> },
}
...
impl SettingField<SharedString> {
  ...
  {
    Self::new(
      SettingFieldType::Element { element: Rc::new(AnySettingFieldElement(element)) },
