use gpui::{
  App, Entity, InteractiveElement as _, IntoElement, ListAlignment, ListState, ParentElement as _,
  SharedString, Styled, Window, div, list, prelude::FluentBuilder as _, px,
};
use rust_i18n::t;
...
impl SettingPage {
  ...
  pub(super) fn render(
    ...
    let groups =
      self.groups.iter().filter(|group| group.is_match(&query)).cloned().collect::<Vec<_>>();
      ...
      .child(
        v_flex()
          ...
          .when_some(self.description.clone(), |this, description| {
            this.child(Label::new(description).text_sm().text_color(cx.theme().muted_foreground))
          }),
      )
      .child(
        div()
          ...
          .child(
            list(list_state.clone(), {
              ...
              move |group_ix, window, cx| {
                ...
                group
                  .py_4()
                  .render(&query, &RenderOptions { page_ix: ix, group_ix, ..options }, window, cx)
                  .into_any_element()
              }
            })
            .size_full(),
