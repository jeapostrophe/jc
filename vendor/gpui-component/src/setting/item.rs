...
pub enum SettingItem {
  ...
  Element { render: Rc<dyn Fn(&RenderOptions, &mut Window, &mut App) -> AnyElement + 'static> },
}
...
impl SettingItem {
  ...
  {
    SettingItem::Element {
      render: Rc::new(move |options, window, cx| render(options, window, cx).into_any_element()),
    }
  }
    ...
    match self {
      SettingItem::Item { title, description, .. } => {
        title.to_lowercase().contains(&query.to_lowercase())
          || description
            .as_ref()
            .map_or(false, |d| d.as_str().to_lowercase().contains(&query.to_lowercase()))
      }
  ...
  fn render_field(
    ...
    let renderer: Box<dyn SettingFieldRender> = match type_id {
      t if t == std::any::TypeId::of::<bool>() => Box::new(BoolField::new(field_type.is_switch())),
  ...
  pub(super) fn render_item(
    ...
    div().id(SharedString::from(format!("item-{}", options.item_ix))).w_full().child(match self {
      SettingItem::Item { title, description, layout, field } => div()
        .w_full()
        .overflow_hidden()
        .map(|this| {
        ...
        .child(
          v_flex()
            .map(
              |this| {
                if layout.is_horizontal() { this.flex_1().max_w_3_5() } else { this.w_full() }
              },
            )
            .gap_1()
        ...
        .into_any_element(),
      SettingItem::Element { render } => (render)(&options, window, cx).into_any_element(),
    })
  }
}
