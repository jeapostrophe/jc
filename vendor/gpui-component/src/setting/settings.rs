...
impl Settings {
      ...
      .filter_map(|page| {
        let filtered_groups: Vec<SettingGroup> = page
          ...
          .filter_map(|group| {
            ...
            group.items =
              group.items.iter().filter(|item| item.is_match(&query)).cloned().collect();
            if group.items.is_empty() { None } else { Some(group) }
          })
          .collect();
        let mut page = page.clone();
        page.groups = filtered_groups;
        if page.groups.is_empty() { None } else { Some(page) }
      })
      .collect()
  }
  ...
  fn render_active_page(
    ...
    for (ix, page) in pages.into_iter().enumerate() {
      if selected_index.page_ix == ix {
        return page.render(ix, state, &options, window, cx).into_any_element();
      }
    }
  ...
  fn render_sidebar(
    ...
    Sidebar::left()
      ...
      .collapsed(false)
      .header(div().w_full().child(Input::new(&search_input).prefix(IconName::Search)))
          ...
          .on_click({
            ...
            move |_, _, cx| {
              state.update(cx, |state, cx| {
                state.selected_index = SelectIndex { page_ix, ..Default::default() };
                cx.notify();
              })
            }
          })
          .when(page.groups.len() > 1, |this| {
            this.children(page.groups.iter().filter(|g| g.title.is_some()).enumerate().map(
              |(group_ix, group)| {
                let is_active =
                  selected_index.page_ix == page_ix && selected_index.group_ix == Some(group_ix);
                ...
                SidebarMenuItem::new(title).active(is_active).on_click({
                  ...
                  move |_, _, cx| {
                    state.update(cx, |state, cx| {
                      state.selected_index = SelectIndex { page_ix, group_ix: Some(group_ix) };
  ...
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    let state = window.use_keyed_state(self.id.clone(), cx, |window, cx| {
      let search_input = cx.new(|cx| {
        InputState::new(window, cx).placeholder(t!("Settings.search_placeholder")).default_value("")
      });
    ...
    h_resizable(self.id.clone())
      .child(resizable_panel().size(self.sidebar_width).child(self.render_sidebar(
        &state,
        &filtered_pages,
        window,
        cx,
