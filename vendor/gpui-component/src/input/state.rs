...
impl InputState {
  ...
  pub fn rows(mut self, rows: usize) -> Self {
    match &mut self.mode {
      ...
      InputMode::AutoGrow { max_rows: max_r, rows: r, .. } => {
        *r = rows;
        *max_r = rows;
      }
    }
  ...
  /// Return the (0-based) start and end [`Position`] of the current selection.
  pub fn selection_positions(&self) -> (Position, Position) {
    let start = self.text.offset_to_position(self.selected_range.start);
    let end = self.text.offset_to_position(self.selected_range.end);
    (start, end)
  }
  ...
  pub(super) fn enter(&mut self, action: &Enter, window: &mut Window, cx: &mut Context<Self>) {
    ...
    if self.mode.is_multi_line() {
      ...
      let indent =
        if self.mode.is_code_editor() { self.indent_of_next_line() } else { "".to_string() };
  ...
  pub(super) fn on_scroll_wheel(
    ...
    let line_height =
      self.last_layout.as_ref().map(|layout| layout.line_height).unwrap_or(window.line_height());
  ...
  pub(crate) fn scroll_to(
    ...
    if let Some(line) = last_layout.lines.get(row.saturating_sub(last_layout.visible_range.start)) {
    ...
    let edge_height =
      if direction.is_some() && self.mode.is_code_editor() { 3 * line_height } else { line_height };
  ...
  fn push_history(&mut self, text: &Rope, range: &Range<usize>, new_text: &str) {
    ...
    self.history.push(Change::new(range.clone(), &old_text, new_range, new_text));
  }
  ...
  pub(crate) fn index_for_mouse_position(&self, position: Point<Pixels>) -> usize {
    ...
    let mut y_offset = last_layout.visible_top;
    for (ix, line) in
      self.text_wrapper.lines.iter().skip(last_layout.visible_range.start).enumerate()
    {
    ...
    let index = if index > self.text.len() { self.text.len() } else { index };
