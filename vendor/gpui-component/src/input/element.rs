...
impl TextElement {
  ...
  fn layout_cursor(
    ...
    for (ix, wrap_line) in text_wrapper.lines.iter().enumerate() {
      ...
      if let Some(line) =
        in_visible_range.then(|| lines.get(ix.saturating_sub(visible_range.start))).flatten()
      {
  ...
  pub(crate) fn layout_match_range(
    ...
    for line in lines.iter() {
      ...
      if line_cursor_start.is_some() || line_cursor_end.is_some() {
        let start =
          line_cursor_start.unwrap_or_else(|| line.position_for_index(0, line_height).unwrap());
  ...
  fn layout_lines(
    ...
    if is_single_line {
      let shaped_line =
        window.text_system().shape_line(display_text.to_string().into(), font_size, &runs, None);
    ...
    if state.text.len() == 0 {
      return display_text
        ...
        .map(|line| {
          let shaped_line =
            window.text_system().shape_line(line.to_string().into(), font_size, &runs, None);
          LineLayout::new().lines(smallvec::smallvec![shaped_line])
        })
        .collect();
    }

    let visible_text = display_text.slice_lines(visible_range.start..visible_range.end).to_string();
    ...
    for (ix, line) in visible_text.split("\n").enumerate() {
      ...
      for range in &line_item.wrapped_lines {
        ...
        let line_runs = if bg_segments.is_empty() {
          ...
          split_runs_by_bg_segments(visible_range_offset.start + offset, &line_runs, bg_segments)
        };
...
impl Element for TextElement {
  ...
  fn prepaint(
    ...
    let marked_run = TextRun {
      ...
      color: text_color,
      background_color: None,
      underline: Some(UnderlineStyle { thickness: px(1.), color: Some(text_color), wavy: false }),
      strikethrough: None,
    };
    ...
    let line_numbers = if state.mode.line_number() {
      ...
      for (ix, line) in last_layout.lines.iter().enumerate() {
        ...
        let runs = if current_row == Some(ix) { &current_line_runs } else { &other_line_runs };
