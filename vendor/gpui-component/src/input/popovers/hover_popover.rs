...
impl HoverPopover {
  pub fn new(
    ...
    cx.new(|_| Self { editor, symbol_range, hover })
  }
...
impl Element for Popover {
  ...
  fn request_layout(
    ...
    let trigger_bounds = match self.trigger_bounds(cx) {
      ...
      None => {
        return (
          ...
          PopoverLayoutState { bounds: Bounds::default(), element: None, state: open_state },
        );
      }
    };

    let max_width =
      self.width_limit.end.min(window.bounds().size.width - SNAP_TO_EDGE * 2).max(px(200.));
    ...
    (
      layout_id,
      PopoverLayoutState {
        bounds: Bounds { origin: pos, size: popover_size },
