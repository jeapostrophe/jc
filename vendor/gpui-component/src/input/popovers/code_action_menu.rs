...
use gpui::{
  Action, AnyElement, App, AppContext, Bounds, Context, DismissEvent, Empty, Entity, EventEmitter,
  InteractiveElement as _, IntoElement, ParentElement, Pixels, Point, Render, RenderOnce,
  SharedString, Styled, StyledText, Subscription, Window, canvas, deferred, div,
...
impl MenuItem {
  fn new(ix: usize, item: Rc<CodeActionItem>) -> Self {
    Self { ix, item, children: vec![], selected: false }
  }
}
...
impl RenderOnce for MenuItem {
  fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    h_flex()
      ...
      .child(div().child(StyledText::new(item.action.title.clone()).with_highlights(highlights)))
...
impl CodeActionMenu {
  ...
  pub(crate) fn new(state: Entity<InputState>, window: &mut Window, cx: &mut App) -> Entity<Self> {
    cx.new(|cx| {
      let view = cx.entity();
      let menu = MenuDelegate { menu: view, items: vec![], selected_ix: 0 };

      let list = cx.new(|cx| ListState::new(menu, window, cx));

      let _subscriptions = vec![cx.subscribe(&list, |this: &mut Self, _, ev: &ListEvent, cx| {
        match ev {
          ListEvent::Confirm(_) => {
            this.hide(cx);
          }
      ...
      Self { offset: 0, state, list, open: false, bounds: Bounds::default(), _subscriptions }
    })
  }
  ...
  fn on_action_down(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    self.list.update(cx, |this, cx| this.on_action_select_next(&actions::SelectDown, window, cx));
  }
