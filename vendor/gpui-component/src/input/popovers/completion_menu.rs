...
impl RenderOnce for CompletionMenuItem {
  fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    let matched_len =
      item.filter_text.as_ref().map(|s| s.len()).unwrap_or(self.highlight_prefix.len());

    let highlights =
      vec![(0..matched_len, HighlightStyle { color: Some(cx.theme().blue), ..Default::default() })];
...
impl CompletionMenu {
  ...
  pub(crate) fn new(editor: Entity<InputState>, window: &mut Window, cx: &mut App) -> Entity<Self> {
    cx.new(|cx| {
      let view = cx.entity();
...
impl Render for CompletionMenu {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let selected_documentation =
      self.list.read(cx).delegate().selected_item().and_then(|item| item.documentation.clone());
