...
impl RopeExt for Rope {
  ...
  fn position_to_offset(&self, pos: &Position) -> usize {
    ...
    self.line_start_offset(pos.line as usize)
      + line.chars().take(pos.character as usize).map(|c| c.len_utf8()).sum::<usize>()
  }
  ...
  fn word_range(&self, offset: usize) -> Option<Range<usize>> {
    ...
    let right =
      self.chars_at(offset).take_while(|c| c.is_alphanumeric() || *c == '_').collect::<String>();
...
mod tests {
  ...
  fn test_iter_lines() {
    let rope = Rope::from("Hello\nWorld\r\nThis is a test 中文\nRope");
    let lines: Vec<_> = rope.iter_lines().skip(1).take(2).map(|r| r.to_string()).collect();
