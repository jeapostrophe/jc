...
use gpui::{
  ...
  IntoElement, IsZero, MouseButton, ParentElement as _, Rems, RenderOnce, StyleRefinement, Styled,
...
impl Input {
  ...
  fn render_editor(
    ...
    let paddings = Edges {
      left: paddings.left.map(|v| v.to_pixels(base_size, rem_size)).unwrap_or(px(0.)),
      right: paddings.right.map(|v| v.to_pixels(base_size, rem_size)).unwrap_or(px(0.)),
      top: paddings.top.map(|v| v.to_pixels(base_size, rem_size)).unwrap_or(px(0.)),
      bottom: paddings.bottom.map(|v| v.to_pixels(base_size, rem_size)).unwrap_or(px(0.)),
    };

    v_flex().size_full().children(state.search_panel.clone()).child(
      div().flex_1().child(input_state.clone()).map(|this| {
    ...
    div()
      ...
      .when(state.mode.is_multi_line(), |this| {
        this
          ...
          .on_action(window.listener_for(&self.state, InputState::recenter_line))
      ...
      .on_mouse_up(MouseButton::Right, window.listener_for(&self.state, InputState::on_mouse_up))
      ...
      .when(self.appearance, |this| {
        this.bg(bg).rounded(cx.theme().radius).when(self.bordered, |this| {
          this
