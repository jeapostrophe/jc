    ...
    match self {
      ...
      Self::Number { separator, .. } => {
        ...
        if !int_part
          .chars()
          .enumerate()
          .all(|(i, ch)| ch.is_ascii_digit() || is_sign(&ch) && i == 0 || Some(ch) == *separator)
        {
          return false;
        }
      ...
      Self::Number { separator, fraction } => {
        if let Some(sep) = *separator {
          ...
          let final_str = if let Some(frac) = frac_part {
            if fraction == &Some(0) { int_with_sep } else { format!("{}.{}", int_with_sep, frac) }
          } else {
            int_with_sep
          };
...
mod tests {
  ...
  fn test_number_with_fraction_digits() {
    let mask = MaskPattern::Number { separator: Some(','), fraction: Some(4) };
    ...
    let mask = MaskPattern::Number { separator: Some(','), fraction: None };

    assert_eq!(mask.mask("1234567.1234567"), "1,234,567.1234567");

    let mask = MaskPattern::Number { separator: Some(','), fraction: Some(0) };
  ...
  fn test_signed_number_numbers() {
    let mask = MaskPattern::Number { separator: Some(','), fraction: Some(2) };
