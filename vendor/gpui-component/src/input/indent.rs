...
impl TabSize {
  pub(super) fn to_string(&self) -> SharedString {
    if self.hard_tabs { "\t".into() } else { " ".repeat(self.tab_size).into() }
  }
  ...
  pub(super) fn indent(&mut self, block: bool, window: &mut Window, cx: &mut Context<Self>) {
    ...
    if is_selected || block {
      ...
      let selected_text = self
        .text_for_range(self.range_to_utf16(&(offset..selected_range.end)), &mut None, window, cx)
        .unwrap_or("".into());
...
mod tests {
  ...
  #[test]
  fn test_tab_size() {
    let tab = TabSize { tab_size: 2, hard_tabs: false };
    assert_eq!(tab.to_string(), "  ");
    let tab = TabSize { tab_size: 4, hard_tabs: false };
    assert_eq!(tab.to_string(), "    ");

    let tab = TabSize { tab_size: 2, hard_tabs: true };
    assert_eq!(tab.to_string(), "\t");
    let tab = TabSize { tab_size: 4, hard_tabs: true };
    assert_eq!(tab.to_string(), "\t");
  }
