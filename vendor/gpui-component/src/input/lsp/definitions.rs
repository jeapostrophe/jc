...
impl InputState {
    ...
    self.lsp._hover_task = cx.spawn_in(window, async move |_, cx| {
      ...
      _ = editor.update(cx, |editor, cx| {
        if locations.is_empty() {
          ...
          editor.hover_definition.update(symbol_range.clone(), locations.clone());
        }
        cx.notify();
      });
  ...
  pub(crate) fn go_to_definition(
    ...
    if location.target_uri.scheme().map(|s| s.as_str() == "https" || s.as_str() == "http")
      == Some(true)
    {
...
impl TextElement {
  pub(crate) fn layout_hover_definition(&self, cx: &App) -> Option<(Range<usize>, HighlightStyle)> {
    ...
    let mut highlight_style: HighlightStyle =
      cx.theme().highlight_theme.link_text.map(|style| style.into()).unwrap_or_default();

    highlight_style.underline =
      Some(UnderlineStyle { thickness: px(1.), ..UnderlineStyle::default() });
