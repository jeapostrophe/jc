...
impl TextWrapper {
  ...
  pub(super) fn update(
    ...
    self._update(changed_text, range, new_text, &mut |line_str, wrap_width| {
      line_wrapper.wrap_line(&[LineFragment::text(line_str)], wrap_width).collect()
    });
  }
    ...
    F: FnMut(&str, Pixels) -> Vec<gpui::Boundary>,
  {
    ...
    let new_end_row = changed_text.offset_to_point(range.start + new_text.len()).row;
    ...
    // line not contains `\n`.
    for (ix, line) in Rope::from(changed_text.slice(new_range)).iter_lines().enumerate() {
      ...
      new_lines.push(LineItem { line: Rope::from(line), wrapped_lines });
    }
    ...
    self.longest_row = LongestRow { row: longest_row_ix, len: longest_row_len }
  }
  ...
  pub(crate) fn offset_to_display_point(&self, offset: usize) -> DisplayPoint {
    ...
    let mut wrapped_row = self.lines.iter().take(row).map(|l| l.lines_len()).sum::<usize>();
    ...
    for (ix, range) in line.wrapped_lines.iter().enumerate() {
      if range.contains(&local_offset) {
        return DisplayPoint::new(wrapped_row + ix, ix, local_offset.saturating_sub(range.start));
      }
    }
...
impl LineLayout {
  pub(crate) fn new() -> Self {
    Self { len: 0, longest_width: px(0.), wrapped_lines: SmallVec::new() }
  }
  ...
  pub(crate) fn set_wrapped_lines(&mut self, wrapped_lines: SmallVec<[ShapedLine; 1]>) {
    ...
    let width = wrapped_lines.iter().map(|l| l.width).max().unwrap_or_default();
  ...
  pub(super) fn paint(
    ...
    for (ix, line) in self.wrapped_lines.iter().enumerate() {
      _ = line.paint(pos + point(px(0.), ix * line_height), line_height, window, cx);
    }
  }
}
...
mod tests {
  ...
  fn test_update() {
    ...
    assert_wrapper_lines(
      &text,
      &wrapper,
      &[&["AAA, 世界!\r"], &["This is third line."], &["这里是第 4 行。New text"]],
    );
    ...
    assert_wrapper_lines(
      &text,
      &wrapper,
      &[&["This is a new line."], &["This is new line 2."], &["这里是第 4 行。New text"]],
    );
  ...
  fn test_offset_to_display_point() {
    ...
    wrapper.lines = vec![
      // range: 0..15
      LineItem { line: Rope::from("Hello, 世界!\r"), wrapped_lines: vec![0..15] },
      // range: 16..36
      LineItem { line: Rope::from("This is second line."), wrapped_lines: vec![0..10, 10..20] },
      // range: 37..56
      LineItem {
      ...
      // range: 57..79
      LineItem { line: Rope::from("这里是第 4 行。"), wrapped_lines: vec![0..22] },
    ];
