...
use gpui::{
  AppContext, Entity, FocusHandle, Focusable, InteractiveElement, IntoElement, KeyBinding, Length,
  MouseButton, ParentElement, Render, Styled, Task, Window, div, prelude::FluentBuilder,
};
use rust_i18n::t;
use smol::Timer;
  ...
  D: ListDelegate,
{
  ...
  fn set_searching(&mut self, searching: bool, window: &mut Window, cx: &mut Context<Self>) {
    self.query_input.update(cx, |input, cx| input.set_loading(searching, window, cx));
  }
  ...
  fn prepare_items_if_needed(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    if let Some(mut el) =
      self.delegate.render_section_header(0, window, cx).map(|r| r.into_any_element())
    {
      measured_size.section_header_size = el.layout_as_root(available_space, window, cx);
    }
    if let Some(mut el) =
      self.delegate.render_section_footer(0, window, cx).map(|r| r.into_any_element())
    {
      measured_size.section_footer_size = el.layout_as_root(available_space, window, cx);
    }

    self.rows_cache.prepare_if_needed(sections_count, measured_size, cx, |section_ix, cx| {
  ...
  fn render_list_item(
    ...
    let mouse_right_clicked = self.mouse_right_clicked_index.map(|s| s.eq_row(ix)).unwrap_or(false);
    ...
    div()
      ...
      .children(
        self
          ...
          .map(|item| item.selected(selected).secondary_selected(mouse_right_clicked)),
      )
      .when(selectable, |this| {
        this
          .on_click(cx.listener(move |this, e: &ClickEvent, window, cx| {
            ...
            this.on_action_confirm(&Confirm { secondary: e.modifiers().secondary() }, window, cx);
          }))
          .on_mouse_down(
            MouseButton::Right,
  ...
  D: ListDelegate,
{
  fn focus_handle(&self, cx: &App) -> FocusHandle {
    if self.searchable { self.query_input.focus_handle(cx) } else { self.focus_handle.clone() }
  }
}
  ...
  D: ListDelegate + 'static,
{
  fn render(mut self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    div().id("list").size_full().refine_style(&self.style).child(self.state.clone())
  }
}
