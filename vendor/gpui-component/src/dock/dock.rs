...
impl Dock {
  pub(crate) fn new(
    ...
    let panel = DockItem::Tabs { size: None, items: Vec::new(), active_ix: 0, view: panel.clone() };
  ...
  pub(super) fn from_state(
    ...
    Self { placement, dock_area, panel, open, size, collapsible: true, resizing: false }
  }
  ...
  pub fn add_panel(
    ...
    self.panel.add_panel(panel, &self.dock_area, None, window, cx);
    cx.notify();
  }
  ...
  fn render_resize_handle(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    resize_handle("resize-handle", axis).placement(self.placement).on_drag(
      ResizePanel {},
      move |info, _, _, cx| {
        cx.stop_propagation();
...
impl Render for Dock {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl gpui::IntoElement {
    ...
    div()
      ...
      .child(DockElement { view: cx.entity().clone() })
  }
}
