...
impl StackPanel {
  pub fn new(axis: Axis, _: &mut Window, cx: &mut Context<Self>) -> Self {
    ...
    let _subscriptions = vec![
      ...
      cx.subscribe(&state, |_, _, _: &ResizablePanelEvent, cx| cx.emit(PanelEvent::LayoutChanged)),
    ];
  ...
  pub(super) fn right_top_tab_panel(
    ...
    let panel = if self.axis.is_vertical() { self.panels.first() } else { self.panels.last() };
...
impl Render for StackPanel {
  fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    h_flex().size_full().overflow_hidden().bg(cx.theme().tab_bar).child(
      ResizablePanelGroup::new("stack-panel-group")
        ...
        .children(
          self
            ...
            .clone()
            .into_iter()
            .map(|panel| resizable_panel().child(panel.view()).visible(panel.visible(cx))),
        ),
    )
  }
}
