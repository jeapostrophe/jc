...
impl Panel for TabPanel {
  ...
  fn closable(&self, cx: &App) -> bool {
    ...
    self.active_panel(cx).map(|panel| panel.closable(cx)).unwrap_or(false)
  }
  ...
  fn toolbar_buttons(
    ...
    self.active_panel(cx).and_then(|panel| panel.toolbar_buttons(window, cx))
  }
  ...
  fn inner_padding(&self, cx: &App) -> bool {
    self.active_panel(cx).map_or(true, |panel| panel.inner_padding(cx))
  }
}
...
impl TabPanel {
  ...
  fn add_panel_with_active(
    ...
    assert_ne!(panel.panel_name(cx), "StackPanel", "can not allows add `StackPanel` to `TabPanel`");

    if self.panels.iter().any(|p| p.view().entity_id() == panel.view().entity_id()) {
      return;
    }
  ...
  fn visible_panels<'a>(&'a self, cx: &'a App) -> impl Iterator<Item = Arc<dyn PanelView>> + 'a {
    self.panels.iter().filter_map(
      |panel| {
        if panel.visible(cx) { Some(panel.clone()) } else { None }
      },
    )
  }
  ...
  fn render_toolbar(
    ...
    h_flex()
      ...
      .when_some(self.toolbar_buttons(window, cx), |this, buttons| {
        this.children(buttons.into_iter().map(|btn| btn.xsmall().ghost().tab_stop(false)))
      })
      .map(|this| {
        let value = if zoomed {
            ...
            move |menu, window, cx| {
              view.update(cx, |this, cx| {
                this
                  ...
                  .separator()
                  .menu_with_disabled(
                    if zoomed { t!("Dock.Zoom Out") } else { t!("Dock.Zoom In") },
                    Box::new(ToggleZoom),
                    !zoomable,
                  )
                  .when(closable, |this| {
                    this.separator().menu(t!("Dock.Close"), Box::new(ClosePanel))
                  })
              })
            }
          })
  ...
  fn render_title_bar(
        ...
        .child(
          div()
            ...
            .when(state.draggable, |this| {
              this.on_drag(DragPanel { panel: panel.clone(), tab_panel: view }, |drag, _, _, cx| {
                cx.stop_propagation();
                cx.new(|_| drag.clone())
              })
            }),
    ...
    TabBar::new("tab-bar")
        ...
        Some(
          Tab::default()
            ...
            .when(!droppable, |this| {
              this
                .when(state.draggable, |this| {
                  this.on_drag(DragPanel::new(panel.clone(), view.clone()), |drag, _, _, cx| {
                    cx.stop_propagation();
                    cx.new(|_| drag.clone())
                  })
                })
      ...
      .last_empty_space(
        ...
        div().id("tab-bar-empty-space").h_full().flex_grow().min_w_16().when(
          state.droppable,
          |this| {
            this.drag_over::<DragPanel>(|this, _, _, cx| this.bg(cx.theme().drop_target)).on_drop(
              cx.listener(move |this, drag: &DragPanel, window, cx| {
                this.will_split_placement = None;

                let ix = if drag.tab_panel == view { Some(tabs_count - 1) } else { None };
      ...
      .when(!self.collapsed, |this| {
        this.suffix(
          h_flex()
            ...
            .px_2()
            .gap_1()
            .children(self.active_panel(cx).and_then(|panel| panel.title_suffix(window, cx)))
            .child(self.render_toolbar(state, window, cx))
    ...
    v_flex()
      ...
      .child(
        div()
          ...
          .overflow_x_hidden()
          .flex_1()
          .child(active_panel.view().cached(StyleRefinement::default().absolute().size_full())),
      )
      .when(state.droppable, |this| {
        this.on_drag_move(cx.listener(Self::on_panel_drag_move)).child(
          div()
            .invisible()
            .absolute()
  ...
  fn split_panel(
    ...
    let ix = stack_panel.read(cx).index_of_panel(Arc::new(cx.entity().clone())).unwrap_or_default();
    ...
    if parent_axis.is_vertical() && placement.is_vertical() {
      ...
      new_stack_panel.update(cx, |view, cx| match placement {
        Placement::Left | Placement::Top => {
          ...
          view.add_panel(Arc::new(tab_panel.clone()), None, dock_area.clone(), window, cx);
        }
      ...
      if stack_panel != new_stack_panel {
        stack_panel.update(cx, |view, cx| {
          view.replace_panel(Arc::new(tab_panel.clone()), new_stack_panel.clone(), window, cx);
        });
      }
