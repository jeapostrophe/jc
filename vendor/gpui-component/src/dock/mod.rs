  ...
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      DockItem::Tabs { items, active_ix, .. } => {
        f.debug_struct("Tabs").field("items", &items.len()).field("active_ix", active_ix).finish()
      }
...
impl DockItem {
  ...
  pub fn split_with_sizes(
    ...
    Self::Split { axis, size: None, items, sizes, view: stack_panel }
  }
  ...
  pub fn tiles(
    ...
    Self::Tiles { size: None, items: tile_panel.read(cx).panels.clone(), view: tile_panel }
  }
  ...
  fn new_tabs(
    ...
    Self::Tabs { size: None, items, active_ix, view: tab_panel }
  }
  ...
  pub fn find_panel(&self, panel: Arc<dyn PanelView>) -> Option<Arc<dyn PanelView>> {
    match self {
      ...
      Self::Tiles { items, .. } => items
        .iter()
        .find_map(|item| if &item.panel == &panel { Some(item.panel.clone()) } else { None }),
    }
  }
...
impl DockArea {
  ...
  fn subscribe_tiles_item_drop(
    ...
    self._subscriptions.push(cx.subscribe(tile_panel, move |_, _, evt: &DragDrop, cx| {
  ...
  pub fn is_dock_open(&self, placement: DockPlacement, cx: &App) -> bool {
    match placement {
      DockPlacement::Left => {
        self.left_dock.as_ref().map(|dock| dock.read(cx).is_open()).unwrap_or(false)
      }
      DockPlacement::Bottom => {
        self.bottom_dock.as_ref().map(|dock| dock.read(cx).is_open()).unwrap_or(false)
      }
      DockPlacement::Right => {
        self.right_dock.as_ref().map(|dock| dock.read(cx).is_open()).unwrap_or(false)
      }
      DockPlacement::Center => false,
    }
  }
  ...
  pub fn is_dock_collapsible(&self, placement: DockPlacement, cx: &App) -> bool {
    match placement {
      DockPlacement::Left => {
        self.left_dock.as_ref().map(|dock| dock.read(cx).collapsible).unwrap_or(false)
      }
      DockPlacement::Bottom => {
        self.bottom_dock.as_ref().map(|dock| dock.read(cx).collapsible).unwrap_or(false)
      }
      DockPlacement::Right => {
        self.right_dock.as_ref().map(|dock| dock.read(cx).collapsible).unwrap_or(false)
      }
      DockPlacement::Center => false,
    }
  }
  ...
  pub fn add_panel(
    ...
    match placement {
      ...
      DockPlacement::Center => {
        self.items.add_panel(panel, &cx.entity().downgrade(), bounds, window, cx);
      }
    }
  }
  ...
  pub fn dump(&self, cx: &App) -> DockAreaState {
    ...
    let left_dock = self.left_dock.as_ref().map(|dock| DockState::new(dock.clone(), cx));
    let right_dock = self.right_dock.as_ref().map(|dock| DockState::new(dock.clone(), cx));
    let bottom_dock = self.bottom_dock.as_ref().map(|dock| DockState::new(dock.clone(), cx));

    DockAreaState { version: self.version, center, left_dock, right_dock, bottom_dock }
  }
  ...
  fn subscribe_item(&mut self, item: &DockItem, window: &mut Window, cx: &mut Context<Self>) {
    match item {
      DockItem::Split { items, view, .. } => {
        ...
        self._subscriptions.push(cx.subscribe_in(view, window, move |_, _, event, window, cx| {
          match event {
  ...
  pub fn update_toggle_button_tab_panels(&mut self, _: &mut Window, cx: &mut Context<Self>) {
    ...
    self.toggle_button_panels.right =
      self.items.right_top_tab_panel(cx).map(|view| view.entity_id());
