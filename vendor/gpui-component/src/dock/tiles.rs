...
impl TileItem {
  pub fn new(panel: Arc<dyn PanelView>, bounds: Bounds<Pixels>) -> Self {
    Self { id: panel.view().entity_id(), panel, bounds, z_index: 0 }
  }
...
impl Panel for Tiles {
  ...
  fn dump(&self, cx: &App) -> PanelState {
    let panels = self.panels.iter().map(|item: &TileItem| item.panel.dump(cx)).collect();

    let metas = self
      .panels
      .iter()
      .map(|item: &TileItem| TileMeta { bounds: item.bounds, z_index: item.z_index })
      .collect();
...
impl Tiles {
  ...
  fn update_position(&mut self, mouse_position: Point<Pixels>, cx: &mut Context<Self>) {
    ...
    let dragging_bounds = Bounds { origin: new_origin, size: self.dragging_initial_bounds.size };

    let (snap_x, snap_y) = self.calculate_magnetic_snap(dragging_bounds, item_ix, snap_threshold);
  ...
  fn resize(
    ...
    let final_x =
      if let Some(x) = new_x { round_to_nearest_ten(x, cx) } else { previous_bounds.origin.x };
    let final_y =
      if let Some(y) = new_y { round_to_nearest_ten(y, cx) } else { previous_bounds.origin.y };
  ...
  pub fn undo(&mut self, _: &mut Window, cx: &mut Context<Self>) {
    ...
    if let Some(changes) = self.history.undo() {
      for change in changes {
        if let Some(index) =
          self.panels.iter().position(|item| item.panel.view().entity_id() == change.tile_id)
        {
        ...
        .on_drag_move(cx.listener(move |this, e: &DragMoveEvent<DragResizing>, window, cx| {
          match e.drag(cx) {
            DragResizing(id) => {
              ...
              let new_height = (drag_data.last_bounds.size.height + delta).max(MINIMUM_SIZE.width);
  ...
  fn on_resize_handle_mouse_down(
    ...
    self.resizing_drag_data = Some(ResizeDrag { side, last_position, last_bounds: item_bounds });
  ...
  fn render_panel(&mut self, item: &TileItem, window: &mut Window, cx: &mut Context<Self>) -> Div {
...
impl Render for Tiles {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let scroll_bounds =
      self.panels.iter().fold(Bounds::default(), |acc: Bounds<Pixels>, item| Bounds {
        origin: Point {
    ...
    div()
      ...
      .child(
        div()
          ...
          .left(-px(1.))
          .overflow_scroll()
          .children(panels.into_iter().map(|item| self.render_panel(&item, window, cx)))
          .child({
