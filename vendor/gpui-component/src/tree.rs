...
impl TreeState {
  ...
  fn add_entry(&mut self, item: TreeItem, depth: usize) {
    self.entries.push(TreeEntry { item: item.clone(), depth });
    if item.is_expanded() {
  ...
  fn rebuild_entries(&mut self) {
    let root_items: Vec<TreeItem> =
      self.entries.iter().filter(|e| e.is_root()).map(|e| e.item.clone()).collect();
    self.entries.clear();
  ...
  fn on_action_up(&mut self, _: &SelectUp, _: &mut Window, cx: &mut Context<Self>) {
    ...
    self.scroll_handle.scroll_to_item(selected_ix, gpui::ScrollStrategy::Top);
    cx.notify();
  }
  ...
  fn on_action_down(&mut self, _: &SelectDown, _: &mut Window, cx: &mut Context<Self>) {
    ...
    self.selected_ix = Some(selected_ix);
    self.scroll_handle.scroll_to_item(selected_ix, gpui::ScrollStrategy::Bottom);
    cx.notify();
  }
...
impl RenderOnce for Tree {
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    self.state.update(cx, |state, _| state.render_item = self.render_item);
