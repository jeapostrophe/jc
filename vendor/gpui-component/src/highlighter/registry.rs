...
impl StatusColors {
  ...
  pub fn error_background(&self, cx: &App) -> Hsla {
    ...
    self.error_background.unwrap_or(bg.blend(self.error(cx).alpha(0.2)))
  }
  ...
  pub fn warning_background(&self, cx: &App) -> Hsla {
    let bg = cx.theme().background;
    self.warning_background.unwrap_or(bg.blend(self.warning(cx).alpha(0.2)))
  }
  ...
  pub fn info_background(&self, cx: &App) -> Hsla {
    let bg = cx.theme().background;
    self.info_background.unwrap_or(bg.blend(self.info(cx).alpha(0.2)))
  }
  ...
  pub fn success_background(&self, cx: &App) -> Hsla {
    let bg = cx.theme().background;
    self.success_background.unwrap_or(bg.blend(self.success(cx).alpha(0.2)))
  }
  ...
  pub fn hint_background(&self, cx: &App) -> Hsla {
    let bg = cx.theme().background;
    self.hint_background.unwrap_or(bg.blend(self.hint(cx).alpha(0.2)))
  }
...
impl LanguageRegistry {
  ...
  pub fn register(&self, lang: &str, config: &LanguageConfig) {
    self.languages.lock().unwrap().insert(lang.to_string().into(), config.clone());
  }
  ...
  pub fn language(&self, name: &str) -> Option<LanguageConfig> {
    ...
    languages.get(name).or_else(|| languages.get(Language::from_str(name).name())).cloned()
  }
}
