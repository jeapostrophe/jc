...
impl From<lsp_types::Diagnostic> for Diagnostic {
  fn from(value: lsp_types::Diagnostic) -> Self {
    Self {
      ...
      severity: value.severity.map(Into::into).unwrap_or(DiagnosticSeverity::Info),
...
impl DiagnosticSeverity {
  ...
  pub(crate) fn highlight_style(&self, cx: &App) -> HighlightStyle {
    ...
    style.underline = Some(UnderlineStyle { color: color, thickness: px(1.), wavy: true });
...
impl sum_tree::Item for DiagnosticEntry {
  ...
  fn summary(&self, _cx: &()) -> Self::Summary {
    DiagnosticSummary { count: 1, start: self.range.start, end: self.range.end }
  }
}
...
impl sum_tree::Summary for DiagnosticSummary {
  ...
  fn zero(_: Self::Context<'_>) -> Self {
    DiagnosticSummary { count: 0, start: usize::MIN, end: usize::MIN }
  }
...
impl DiagnosticSet {
  pub fn new(text: &Rope) -> Self {
    Self { text: text.clone(), diagnostics: SumTree::new(&()) }
  }
  ...
  pub fn push(&mut self, diagnostic: impl Into<Diagnostic>) {
    ...
    self.diagnostics.push(DiagnosticEntry { range: start..end, diagnostic }, &());
  }
