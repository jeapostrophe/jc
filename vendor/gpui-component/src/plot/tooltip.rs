...
impl CrossLine {
  pub fn new(point: Point<Pixels>) -> Self {
    Self { point, height: None, direction: Default::default() }
  }
  ...
  fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    div()
      ...
      .when(self.direction.show_vertical(), |this| {
        this.child(div().absolute().w(px(1.)).bg(cx.theme().border).top_0().left(self.point.x).map(
          |this| {
            if let Some(height) = self.height { this.h(px(height)) } else { this.h_full() }
          },
        ))
      })
      .when(self.direction.show_horizontal(), |this| {
        this.child(
          div().absolute().w_full().h(px(1.)).bg(cx.theme().border).left_0().top(self.point.y),
        )
      })
  }
}
...
impl Dot {
  pub fn new(point: Point<Pixels>) -> Self {
    Self { point, size: px(6.), stroke: gpui::transparent_black(), fill: gpui::transparent_black() }
  }
...
impl TooltipState {
  pub fn new(
    ...
    Self { index, cross_line, dots, position }
  }
}
