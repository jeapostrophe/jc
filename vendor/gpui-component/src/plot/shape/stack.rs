...
impl<T: Clone> Default for Stack<T> {
  fn default() -> Self {
    Self { data: Vec::new(), keys: Vec::new(), value: Box::new(|_, _| None) }
  }
}
...
impl<T: Clone> Stack<T> {
  ...
  pub fn series(&self) -> Vec<StackSeries<T>> {
    ...
    for (i, key) in self.keys.iter().enumerate() {
      ...
      result.push(StackSeries { key: key.clone(), index: i, points });
    }
...
mod tests {
  ...
  fn test_basic_stack() {
    let data = vec![
      SalesData { date: "Jan".to_string(), apples: 10.0, bananas: 20.0, cherries: 30.0 },
      SalesData { date: "Feb".to_string(), apples: 15.0, bananas: 25.0, cherries: 35.0 },
    ];

    let stack = Stack::new().data(data).keys(vec!["apples", "bananas", "cherries"]).value(
      |d, key| match key {
