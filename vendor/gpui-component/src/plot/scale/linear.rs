  ...
  T: Copy + PartialOrd + Num + ToPrimitive + Sealed,
{
  ...
  fn least_index_with_domain(&self, tick: f32, domain: &[T]) -> (usize, f32) {
    ...
    domain
      ...
      .enumerate()
      .min_by(|(_, a), (_, b)| {
        ((*a) - tick).abs().partial_cmp(&((*b) - tick).abs()).unwrap_or(std::cmp::Ordering::Equal)
      })
      .unwrap_or((0, 0.))
  }
}
