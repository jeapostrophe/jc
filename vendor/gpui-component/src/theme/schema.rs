...
impl ThemeColor {
  ...
  pub(crate) fn apply_config(&mut self, config: &ThemeConfig, default_theme: &ThemeColor) {
    ...
    apply_color!(info_hover, fallback = self.background.blend(self.info.opacity(hover_opacity)));
    ...
    apply_color!(list_active_border, fallback = self.background.blend(self.primary.opacity(0.6)));
