...
pub(super) fn init(cx: &mut App) {
  ...
  cx.observe_global::<ThemeRegistry>(|cx| {
    ...
    if let Some(theme) = ThemeRegistry::global(cx).themes().get(&light_theme).cloned() {
    ...
    let theme_name = if mode.is_dark() { dark_theme } else { light_theme };
...
impl ThemeRegistry {
  ...
  pub fn sorted_themes(&self) -> Vec<&Rc<ThemeConfig>> {
    ...
    themes.sort_by(|a, b| {
      b.is_default.cmp(&a.is_default).then(a.name.to_lowercase().cmp(&b.name.to_lowercase()))
    });
    themes
  }
  ...
  fn reload(&mut self) -> Result<()> {
      ...
      for entry in fs::read_dir(&self.themes_dir)? {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("json") {
          ...
          match serde_json::from_str::<ThemeSet>(&file_content) {
            ...
            Err(e) => {
              tracing::error!("ignored invalid theme file: {}, {}", path.display(), e);
            }
          }
        }
      }
    ...
    for theme in themes.iter() {
      ...
      if theme.is_default {
        self.default_themes.insert(theme.mode, Rc::new(theme.clone()));
      }
