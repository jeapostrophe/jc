...
impl<'a> Context<'a> {
  ...
  fn trim_right(&self) -> bool {
    self
      .right
      .map_or(true, |siblings| siblings.iter().find_map(Self::is_block_element).unwrap_or(true))
  }

  fn next_element(&self) -> Option<&Rc<Node>> {
    self.right.and_then(|siblings| {
      siblings.iter().find(|node| matches!(node.data, NodeData::Element { .. }))
    })
  }
  ...
  W: io::Write,
{
  ...
  pub fn minify<R: io::Read>(&mut self, mut r: &mut R) -> io::Result<()> {
    let dom =
      parse_document(RcDom::default(), ParseOpts::default()).from_utf8().read_from(&mut r)?;
    ...
    match &node.data {
      NodeData::Text { contents } => {
        ...
        let (trim_left, trim_right) =
          ctx.as_ref().map_or((true, true), |ctx| ctx.trim(self.preceding_whitespace));
  ...
  fn is_whitespace(&self, s: &RefCell<Tendril<UTF8>>) -> Option<bool> {
    if self.collapse_whitespace && is_whitespace(s) {
      ...
      Some(!s.borrow().as_bytes().iter().next().map_or(false, u8::is_ascii_whitespace))
    }
  }
  ...
  fn write_attribute(&mut self, attr: &Attribute) -> io::Result<()> {
    ...
    let value =
      if self.collapse_whitespace { value.trim_matches(is_ascii_whitespace) } else { value };
    ...
    let (unquoted, double, _) =
      b.iter().fold((true, false, false), |(unquoted, double, single), &c| {
