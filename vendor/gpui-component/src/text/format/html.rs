...
fn cleanup_html(source: &str) -> Vec<u8> {
  ...
  if let Ok(()) = minify.minify(&mut r) { w.into_inner() } else { source.bytes().collect() }
}
...
fn attr_value(attrs: &RefCell<Vec<html5ever::Attribute>>, name: LocalName) -> Option<String> {
  attrs
    .borrow()
    .iter()
    .find_map(|attr| if attr.name.local == name { Some(attr.value.to_string()) } else { None })
}
...
fn value_to_length(value: &str) -> Option<DefiniteLength> {
  if value.ends_with("%") {
    value.trim_end_matches("%").parse::<f32>().ok().map(|v| relative(v / 100.))
  } else {
    value.trim_end_matches("px").parse().ok().map(|v| px(v).into())
  }
}
...
fn parse_table_cell(
  ...
  let table_cell = node::TableCell { children: paragraph, width };
      ...
      local_name!("a") => {
        ...
        marks.push((
          ...
          TextMark::default().link(LinkMark {
            url: attr_value(&attrs, local_name!("href")).unwrap_or_default().into(),
  ...
  match node.data {
    ...
    NodeData::Element { ref name, ref attrs, .. } => match name.local {
      ...
      | local_name!("h6") => {
        ...
        let level = name.local.chars().last().unwrap_or('6').to_digit(10).unwrap_or(6) as u8;
        ...
        let heading = node::Node::Heading { level, children: paragraph };
        if children.len() > 0 {
      ...
      local_name!("li") => {
        ...
        Some(node::Node::ListItem { children, spread: false, checked: None })
      }
      local_name!("table") => {
        let mut children = vec![];
        consume_paragraph(&mut children, paragraph);
        ...
        for child in node.children.borrow().iter() {
          match child.data {
            NodeData::Element { ref name, .. }
              if name.local == local_name!("tbody") || name.local == local_name!("thead") =>
            {
      ...
      _ => {
        if BLOCK_ELEMENTS.contains(&name.local.trim()) {
          ...
          if children.is_empty() { None } else { Some(node::Node::Root { children }) }
        } else {
          // Others to as Inline
          parse_paragraph(paragraph, node);

          if paragraph.is_image() { Some(node::Node::Paragraph(paragraph.take())) } else { None }
        }
      }
    },
    NodeData::Document => {
