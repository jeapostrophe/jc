...
impl ImageNode {
  pub fn title(&self) -> String {
    self.title.clone().unwrap_or_else(|| self.alt.clone().unwrap_or_default()).to_string()
  }
}
...
impl PartialEq for Paragraph {
  fn eq(&self, other: &Self) -> bool {
    self.span == other.span && self.children == other.children && self.link_refs == other.link_refs
  }
}
...
impl Paragraph {
  ...
  pub(crate) fn push_str(&mut self, text: &str) {
    self
      .children
      .push(InlineNode::new(text.to_string()).marks(vec![(0..text.len(), TextMark::default())]));
  }
  ...
  pub(crate) fn is_empty(&self) -> bool {
    self.children.is_empty()
      || self.children.iter().all(|node| node.text.is_empty() && node.image.is_none())
  }

  /// Return length of children text.
  pub(crate) fn text_len(&self) -> usize {
    self.children.iter().map(|node| node.text.len()).sum::<usize>()
  }
...
impl Paragraph {
  fn render(&self, node_cx: &NodeContext, _window: &mut Window, cx: &mut App) -> impl IntoElement {
    let span = self.span;
    ...
    for inline_node in children {
      ...
      if let Some(image) = &inline_node.image {
        if text.len() > 0 {
          inline_node.state.lock().unwrap().set_text(text.clone().into());
          child_nodes.push(
            Inline::new(ix, inline_node.state.clone(), links.clone(), highlights.clone())
              .into_any_element(),
          );
        }
        child_nodes.push(
        ...
        for (range, style) in &inline_node.marks {
          ...
          if style.strikethrough {
            highlight.strikethrough =
              Some(gpui::StrikethroughStyle { thickness: gpui::px(1.), ..Default::default() });
          }
          if style.code {
          ...
          if let Some(mut link_mark) = style.link.clone() {
            ...
            highlight.underline =
              Some(gpui::UnderlineStyle { thickness: gpui::px(1.), ..Default::default() });
    ...
    if text.len() > 0 {
      ...
      child_nodes.push(Inline::new(ix, self.state.clone(), links, highlights).into_any_element());
    }
    ...
    let mut text = self
      ...
      .map(|text_node| {
        ...
        if let Some(image) = &text_node.image {
          ...
          let title = image.title.clone().map_or(String::new(), |t| format!(" \"{}\"", t));
...
impl Node {
  ...
  pub(crate) fn to_markdown(&self) -> String {
    match self {
      Node::Root { children } => {
        children.iter().map(|child| child.to_markdown()).collect::<Vec<_>>().join("\n\n")
      }
      ...
      Node::ListItem { children, checked, .. } => {
        let checkbox =
          if let Some(checked) = checked { if *checked { "[x] " } else { "[ ] " } } else { "" };
        format!(
          "{}{}",
          checkbox,
          children.iter().map(|child| child.to_markdown()).collect::<Vec<_>>().join("\n")
      ...
      Node::CodeBlock(code_block) => {
        format!("```{}\n{}\n```", code_block.lang.clone().unwrap_or_default(), code_block.code())
      }
      Node::Table(table) => {
        let header = table
          .children
...
impl Node {
  fn render_list_item(
    ...
    match item {
      Node::ListItem { children, spread, checked } => v_flex()
        .id("li")
        ...
        .children({
          ...
          for (child_ix, child) in children.iter().enumerate() {
            match child {
              Node::Paragraph(_) => {
                let last_not_list =
                  child_ix > 0 && !matches!(children[child_ix - 1], Node::List { .. });
                  ...
                  div()
                    ...
                    .children({
                      ...
                      for (ix, cell) in row.children.iter().enumerate() {
                        ...
                        let len = col_lens.get(ix).copied().unwrap_or(MAX_LENGTH).min(MAX_LENGTH);
  ...
  pub(super) fn render_root(
    ...
    let options = NodeRenderOptions { is_last: true, ..Default::default() };

    let Some(list_state) = list_state else {
      return self.render_block(options, node_cx, window, cx).into_any_element();
    };
    ...
    gpui::list(list_state, move |ix, window, cx| {
      ...
      children[ix].render_block(options.is_last(is_last), &node_cx, window, cx).into_any_element()
    })
    .size_full()
    .into_any()
  }
  ...
  fn render_block(
    ...
    let mb =
      if options.in_list || options.is_last { rems(0.) } else { node_cx.style.paragraph_gap };

    match self {
      Node::Root { children } => div()
        .id("div")
        .children(
          children.into_iter().map(move |node| node.render_block(options, node_cx, window, cx)),
        )
        .into_any_element(),
      Node::Paragraph(paragraph) => {
        div().id("p").pb(mb).child(paragraph.render(node_cx, window, cx)).into_any_element()
      }
      Node::Heading { level, children } => {
        ...
        .children({
          ...
          for item in children.into_iter() {
            ...
            items.push(Self::render_list_item(
              item,
              ix,
              NodeRenderOptions { ordered: *ordered, ..options },
              node_cx,
              window,
              cx,
            ));
      ...
      Node::Divider => {
        div().pb(mb).child(div().id("divider").bg(cx.theme().border).h(px(2.))).into_any_element()
      }
