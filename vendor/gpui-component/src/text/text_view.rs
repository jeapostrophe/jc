...
impl RenderOnce for TextViewElement {
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    self.state.update(cx, |state, cx| {
      v_flex().size_full().map(|this| match &mut state.parsed_result {
        ...
        Some(Err(err)) => {
          this.child(v_flex().gap_1().child("Failed to parse content").child(err.to_string()))
        }
        None => this,
      })
    })
...
impl TextViewState {
  ...
  pub(crate) fn has_selection(&self) -> bool {
    if let (Some(start), Some(end)) = self.selection_positions { start != end } else { false }
  }
  ...
  fn selection_text(&self) -> Option<String> {
    Some(self.parsed_result.as_ref()?.as_ref().ok()?.root_node.selected_text())
  }
}
...
impl TextView {
  ...
  pub fn markdown(
    ...
    let state = window.use_keyed_state(SharedString::from(format!("{}/state", id)), cx, |_, cx| {
      TextViewState::new(cx)
    });
    let init_state =
      Self::create_init_state(TextViewType::Markdown, &markdown, &highlight_theme, &state, cx);
    ...
    E: IntoElement,
  {
    self.code_block_actions =
      Some(Arc::new(move |code_block, window, cx| f(&code_block, window, cx).into_any_element()));
    self
  }
}
...
impl Element for TextView {
  ...
  fn request_layout(
    ...
    if let Some(InitState::Initializing { type_, text, style, highlight_theme }) =
    ...
    let mut el = div()
      ...
      .child(TextViewElement {
        list_state: if self.scrollable { Some(list_state.clone()) } else { None },
  ...
  fn paint(
    ...
    GlobalState::global_mut(cx).text_view_state_stack.push(self.state.clone());
...
fn selection_bounds(
  ...
  if let (Some(start), Some(end)) = (start, end) {
    ...
    let origin = Point { x: start.x.min(end.x), y: start.y.min(end.y) };
    let size = Size { width: (start.x - end.x).abs(), height: (start.y - end.y).abs() };
...
mod tests {
  ...
  fn test_text_view_state_selection_bounds() {
    ...
    assert_eq!(
      ...
      Bounds { origin: point(px(10.), px(10.)), size: size(px(40.), px(40.)) }
    );
    // 10,10
    //   |------|
    //   |      |
