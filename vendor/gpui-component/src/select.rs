use gpui::{
  AnyElement, App, AppContext, Bounds, ClickEvent, Context, DismissEvent, Edges, ElementId, Entity,
  ...
  StyleRefinement, Styled, Subscription, Task, WeakEntity, Window, anchored, canvas, deferred, div,
  ...
  D: SelectDelegate + 'static,
{
  ...
  pub fn new(
    ...
    let delegate = SelectListDelegate { delegate, state: cx.entity().downgrade(), selected_index };
  ...
  fn display_title(&mut self, _: &Window, cx: &mut Context<Self>) -> impl IntoElement {
    let default_title = div()
      ...
      .child(self.options.placeholder.clone().unwrap_or_else(|| t!("Select.placeholder").into()))
    ...
    let Some(title) = self.list.read(cx).delegate().delegate.item(*selected_index).map(|item| {
              ...
              div()
                ...
                .child(
                  v_flex()
                    ...
                    .child(
                      List::new(&self.list)
                        .when_some(self.options.search_placeholder.clone(), |this, placeholder| {
  ...
  fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    h_flex()
      ...
      .when(!self.disabled, |this| {
        this.when(!self.selected, |this| this.hover(|this| this.bg(cx.theme().accent.alpha(0.7))))
      })
