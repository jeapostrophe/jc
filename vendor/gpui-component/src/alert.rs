...
impl Alert {
  ...
  pub fn info(id: impl Into<ElementId>, message: impl Into<Text>) -> Self {
    Self::new(id, message).with_variant(AlertVariant::Info).icon(IconName::Info)
  }
  ...
  pub fn success(id: impl Into<ElementId>, message: impl Into<Text>) -> Self {
    Self::new(id, message).with_variant(AlertVariant::Success).icon(IconName::CircleCheck)
  }
  ...
  pub fn warning(id: impl Into<ElementId>, message: impl Into<Text>) -> Self {
    Self::new(id, message).with_variant(AlertVariant::Warning).icon(IconName::TriangleAlert)
  }
  ...
  pub fn error(id: impl Into<ElementId>, message: impl Into<Text>) -> Self {
    Self::new(id, message).with_variant(AlertVariant::Error).icon(IconName::CircleX)
  }
    ...
    h_flex()
      ...
      .child(
        div()
          ...
          .overflow_hidden()
          .gap(gap)
          .child(div().when(!self.banner, |this| this.mt(px(5.))).child(self.icon))
          .child(
            div()
              .flex_1()
              .overflow_hidden()
              ...
              .when(!self.banner, |this| {
                this.when_some(self.title, |this, title| {
                  this.child(div().w_full().truncate().font_semibold().child(title))
                })
              })
              .child(self.message.style(TextViewStyle::default().paragraph_gap(rems(0.2)))),
          ),
      )
      .when_some(self.on_close, |this, on_close| {
        this.child(
          div()
            ...
            .child(
              Icon::new(IconName::Close).with_size(self.size.max(Size::Medium)).flex_shrink_0(),
            ),
        )
      })
      .into_any_element()
