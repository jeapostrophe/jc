...
impl TabVariant {
  ...
  fn inner_paddings(&self, size: Size) -> Edges<Pixels> {
    ...
    Edges { left: padding_x, right: padding_x, ..Default::default() }
  }

  fn inner_margins(&self, size: Size) -> Edges<Pixels> {
    match size {
      Size::XSmall => match self {
        TabVariant::Underline => Edges { top: px(1.), bottom: px(2.), ..Default::default() },
        _ => Edges::all(px(0.)),
      },
      Size::Small => match self {
        TabVariant::Underline => Edges { top: px(2.), bottom: px(3.), ..Default::default() },
        _ => Edges::all(px(0.)),
      },
      Size::Large => match self {
        TabVariant::Underline => Edges { top: px(5.), bottom: px(6.), ..Default::default() },
        _ => Edges::all(px(0.)),
      },
      _ => match self {
        TabVariant::Underline => Edges { top: px(3.), bottom: px(4.), ..Default::default() },
        _ => Edges::all(px(0.)),
      },
    }
  }
  ...
  fn normal(&self, cx: &App) -> TabStyle {
    match self {
      TabVariant::Tab => TabStyle {
        ...
        borders: Edges { top: px(1.), left: px(1.), right: px(1.), ..Default::default() },
  ...
  fn hovered(&self, selected: bool, cx: &App) -> TabStyle {
    match self {
      ...
      TabVariant::Segmented => TabStyle {
        fg: cx.theme().tab_foreground,
        bg: cx.theme().transparent,
        inner_bg: if selected { cx.theme().background } else { cx.theme().transparent },
        inner_radius: cx.theme().radius,
        ..Default::default()
      },
      TabVariant::Underline => TabStyle {
  ...
  fn disabled(&self, selected: bool, cx: &App) -> TabStyle {
    match self {
      TabVariant::Tab => TabStyle {
        ...
        bg: cx.theme().transparent,
        border_color: if selected { cx.theme().border } else { cx.theme().transparent },
        borders: Edges { top: px(1.), left: px(1.), right: px(1.), ..Default::default() },
        ..Default::default()
      },
      TabVariant::Outline => TabStyle {
      ...
      TabVariant::Pill => TabStyle {
        ...
        bg: if selected { cx.theme().primary.opacity(0.5) } else { cx.theme().transparent },
        radius: px(99.),
        ..Default::default()
      },
      TabVariant::Segmented => TabStyle {
...
impl RenderOnce for Tab {
  fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    let mut tab_style =
      if self.selected { self.variant.selected(cx) } else { self.variant.normal(cx) };
      ...
      .child(
        h_flex()
          ...
          .map(|this| match self.icon {
            Some(icon) => this.w(inner_height * 1.25).child(icon.map(|this| match self.size {
          ...
          .hover(|this| this.bg(hover_style.inner_bg).rounded(hover_style.inner_radius)),
      )
