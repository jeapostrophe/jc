...
impl DivInspector {
  pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
    ...
    let json_input_state =
      cx.new(|cx| InputState::new(window, cx).code_editor("json").line_number(false));
    ...
    let rust_state = EditorState { state: rust_input_state, error: None, editing: false };

    let json_state = EditorState { state: json_input_state, error: None, editing: false };
...
impl StyleMethods {
  fn get() -> &'static Self {
    ...
    STYLE_METHODS.get_or_init(|| {
      ...
      .collect();
      let map = table.iter().map(|(_, method)| (method.name, method.clone())).collect();
...
fn style_to_rust(input_style: &StyleRefinement) -> (String, StyleRefinement) {
  let methods: Vec<_> = StyleMethods::get()
    ...
    .filter_map(
      |(style, method)| {
        if input_style.is_superset_of(style) { Some(method) } else { None }
      },
    )
    .collect();
        ...
        this
          ...
          .child(
            v_flex()
              ...
              .h_2_5()
              .gap_y_3()
              .child(h_flex().justify_between().gap_x_2().child("Rust Styles").child(
                Button::new("rust-reset").label("Reset").small().on_click(cx.listener(
                  |this, _, window, cx| {
          ...
          .child(
            v_flex()
              ...
              .h_2_5()
              .flex_shrink_0()
              .child(h_flex().gap_x_2().child(div().flex_1().child("JSON Styles")).child(
                Button::new("json-reset").label("Reset").small().on_click(cx.listener(
                  |this, _, window, cx| {
                    this.reset_style(window, cx);
                  },
                )),
  ...
  v_flex()
    ...
    .child(
      h_flex()
        ...
        .child(Button::new("close").icon(IconName::Close).small().ghost().on_click(
          |_, window, cx| {
          ...
          if name.starts_with(&prefix) {
            Some(CompletionItem {
              ...
              text_edit: Some(CompletionTextEdit::Edit(TextEdit {
                range: lsp_types::Range { start: start_pos, end: end_pos },
