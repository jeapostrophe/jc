...
pub enum DescriptionItem {
  Item { label: DescriptionText, value: DescriptionText, span: usize },
  Divider,
}
...
impl DescriptionItem {
  ...
  pub fn new(label: impl Into<DescriptionText>) -> Self {
    DescriptionItem::Item { label: label.into(), value: "".into(), span: 1 }
  }
...
impl DescriptionList {
  ...
  pub fn item(
    ...
    self.items.push(DescriptionItem::Item { label: label.into(), value: value.into(), span });
    self
  }
  ...
  pub fn children(
    ...
    self.items.extend(children.into_iter().map(Into::into).collect::<Vec<_>>());
    self
  }
...
impl RenderOnce for DescriptionList {
  fn render(self, _: &mut Window, cx: &mut gpui::App) -> impl gpui::IntoElement {
    ...
    let label_width = if self.layout.is_horizontal() { Some(self.label_width) } else { None };
    if !self.bordered {
      padding_x = px(0.);
      padding_y = px(0.);
    }
    ...
    v_flex()
      ...
      .when(self.bordered, |this| {
        this.rounded(padding_x).border_1().border_color(cx.theme().border)
      })
      .children(rows.into_iter().enumerate().map(|(ix, items)| {
        let is_last = ix == rows_len - 1;
        h_flex()
          .when(self.bordered && !is_last, |this| this.border_b_1().border_color(cx.theme().border))
          .children({
              ...
              match item {
                DescriptionItem::Item { label, value, .. } => {
                  ...
                  el.flex_1()
                    ...
                    .child(
                      div()
                        .when(self.layout.is_horizontal(), |this| this.h_full())
                        ...
                        .when(self.bordered, |this| {
                          this
                            .when(self.layout.is_horizontal(), |this| {
                              this.border_r_1().when(!is_first_col, |this| this.border_l_1())
                            })
                            .when(self.layout.is_vertical(), |this| this.border_b_1())
                    ...
                    .child(
                      div().flex_1().px(padding_x).py(padding_y).overflow_hidden().child(value),
                    )
                }
                _ => div()
                  .h_2()
                  .w_full()
                  .when(self.bordered, |this| this.bg(cx.theme().description_list_label)),
              }
            })
          })
      }))
