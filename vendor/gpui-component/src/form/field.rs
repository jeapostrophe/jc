...
impl Field {
  ...
  {
    self.label =
      Some(FieldBuilder::Element(Rc::new(move |window, cx| label(window, cx).into_any_element())));
    self
  }
...
impl RenderOnce for Field {
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    let label_width = if layout.is_vertical() { None } else { self.props.label_width };
    let has_label = self.label_indent;

    #[inline]
    fn wrap_div(layout: Axis) -> Div {
      if layout.is_vertical() { v_flex() } else { h_flex() }
    }
    ...
    let inner_gap = if layout.is_horizontal() { gap } else { gap / 2. };
    ...
    v_flex()
      ...
      .child(
        ...
        wrap_div(layout)
          ...
          .when(has_label, |this| {
            ...
            this.child(
              wrap_label(label_width)
                .text_sm()
                .when_some(self.props.label_text_size, |this, size| this.text_size(size))
                .font_medium()
                .gap_1()
                .items_center()
          ...
          .child(div().w_full().flex_1().overflow_x_hidden().children(self.children)),
      )
      .child(
        // Other
        wrap_div(layout)
