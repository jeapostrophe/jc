...
use gpui::{
  ...
  EventEmitter, FocusHandle, Focusable, InteractiveElement, IntoElement, KeyBinding, ParentElement,
  Pixels, Render, ScrollHandle, SharedString, StatefulInteractiveElement, Styled, WeakEntity,
  Window, anchored, canvas, div, prelude::FluentBuilder, px, rems,
};
...
pub enum PopupMenuItem {
  ...
  Submenu { icon: Option<Icon>, label: SharedString, disabled: bool, menu: Entity<PopupMenu> },
}
...
impl PopupMenuItem {
  ...
  pub fn submenu(label: impl Into<SharedString>, menu: Entity<PopupMenu>) -> Self {
    PopupMenuItem::Submenu { icon: None, label: label.into(), disabled: false, menu }
  }
...
impl PopupMenu {
  ...
  pub fn link_with_disabled(
    ...
    let href = href.into();
    self.menu_items.push(PopupMenuItem::link(label, href).disabled(disabled));
    self
  }
  ...
  fn link_with_icon_and_disabled(
    ...
    let href = href.into();
    self.menu_items.push(PopupMenuItem::link(label, href).icon(icon).disabled(disabled));
    self
  }
    ...
    E: IntoElement,
  {
    self
      .menu_items
      .push(PopupMenuItem::element(builder).action(action).icon(icon).disabled(disabled));
    self
  }
    ...
    E: IntoElement,
  {
    self
      .menu_items
      .push(PopupMenuItem::element(builder).action(action).checked(checked).disabled(disabled));
    self
  }
  ...
  fn clickable_menu_items(&self) -> impl Iterator<Item = (usize, &PopupMenuItem)> {
    self.menu_items.iter().enumerate().filter(|(_, item)| item.is_clickable())
  }
  ...
  fn select_up(&mut self, _: &SelectUp, _: &mut Window, cx: &mut Context<Self>) {
    ...
    if let Some((prev_ix, _)) =
      self.menu_items.iter().enumerate().rev().find(|(i, item)| *i < ix && item.is_clickable())
    {
  ...
  fn select_down(&mut self, _: &SelectDown, _: &mut Window, cx: &mut Context<Self>) {
    ...
    if let Some((next_ix, _)) =
      self.menu_items.iter().enumerate().find(|(i, item)| *i > ix && item.is_clickable())
    {
  ...
  fn render_key_binding(
    ...
    .map(|this| this.p_0().flex_nowrap().border_0().bg(gpui::transparent_white()))
  }
  ...
  fn render_item(
    ...
    match item {
      PopupMenuItem::Separator => {
        this.h_auto().p_0().my_0p5().mx_neg_1().h(px(1.)).bg(cx.theme().border).disabled(true)
      }
      ...
      PopupMenuItem::ElementItem { render, icon, disabled, .. } => this
        .when(!disabled, |this| {
      ...
      PopupMenuItem::Item { icon, label, action, disabled, is_link, .. } => {
          ...
          .child(
            h_flex()
              ...
              .when(show_link_icon, |this| {
                this.child(
                  h_flex().w_full().justify_between().gap_1p5().child(label.clone()).child(
      ...
      PopupMenuItem::Submenu { icon, label, menu, disabled } => this
        .selected(selected)
        .disabled(*disabled)
        .items_start()
        .child(
...
impl Render for PopupMenu {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let has_left_icon = self.menu_items.iter().any(|item| item.has_left_icon(self.check_side));
      ...
      .child(
        v_flex()
          ...
          .when(self.scrollable, |this| {
            this.max_h(max_height).overflow_y_scroll().track_scroll(&self.scroll_handle)
          })
          .children(
            self
              .menu_items
          ...
          .child({
            canvas(move |bounds, _, cx| view.update(cx, |r, _| r.bounds = bounds), |_, _, _, _| {})
              .absolute()
              .size_full()
          }),
      )
