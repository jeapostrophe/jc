...
impl<E: ParentElement + Styled> Styled for ContextMenu<E> {
  fn style(&mut self) -> &mut StyleRefinement {
    if let Some(element) = &mut self.element { element.style() } else { &mut self._ignore_style }
  }
}
  ...
  fn request_layout(
    ...
    self.with_element_state(
      ...
      |this, state: &mut ContextMenuState, window, cx| {
        ...
        if open {
          let has_menu_item =
            menu_view.as_ref().map(|menu| !menu.read(cx).is_empty()).unwrap_or(false);
        ...
        (layout_id, ContextMenuState { element: Some(element), ..Default::default() })
      },
    )
  }
