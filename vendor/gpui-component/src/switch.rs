...
impl RenderOnce for Switch {
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    let inset = px(2.);
    let radius = if cx.theme().radius >= px(4.) { bg_height } else { cx.theme().radius };
      ...
      h_flex()
        ...
        .child(
          ...
          div()
            ...
            .child(
              // Switch Toggle
              div().rounded(radius).bg(toggle_bg).shadow_md().size(bar_width).map(|this| {
        ...
        .when_some(self.label, |this, label| {
          this.child(div().line_height(bg_height).child(label).map(|this| match self.size {
