...
impl SliderState {
  ...
  fn update_value_by_position(
    ...
    let inner_pos =
      if axis.is_horizontal() { position.x - bounds.left() } else { bounds.bottom() - position.y };
...
impl Slider {
  ...
  fn render_thumb(
    ...
    div()
      ...
      .size_4()
      .p(px(1.))
      .child(div().flex_shrink_0().size_full().corner_radii(radius).bg(thumb_color))
        ...
        h_flex()
          ...
          .when(!self.disabled, |this| {
            this.on_mouse_down(
              ...
              window.listener_for(&self.state, move |state, e: &MouseDownEvent, window, cx| {
                ...
                state.update_value_by_position(axis, e.position, is_start, window, cx)
              }),
            )
          })
              ...
              .on_drag_move(window.listener_for(
                ...
                move |view, e: &DragMoveEvent<DragSlider>, window, cx| match e.drag(cx) {
                  DragSlider(id) => {
                    ...
                    view.update_value_by_position(axis, e.event.position, false, window, cx)
                  }
                },
              ))
          })
          ...
          .child(
            div()
              ...
              .child(self.render_thumb(bar_end, false, bar_color, thumb_color, radius, window, cx))
              .child({
