...
impl TitleBar {
  ...
  pub fn new() -> Self {
    Self { style: StyleRefinement::default(), children: SmallVec::new(), on_close_window: None }
  }
...
enum ControlIcon {
  ...
  Restore,
  Maximize,
  Close { on_close_window: Option<Rc<Box<dyn Fn(&ClickEvent, &mut Window, &mut App)>>> },
}
...
impl ControlIcon {
  ...
  fn hover_fg(&self, cx: &App) -> Hsla {
    if self.is_close() { cx.theme().danger_foreground } else { cx.theme().secondary_foreground }
  }

  #[inline]
  fn hover_bg(&self, cx: &App) -> Hsla {
    if self.is_close() { cx.theme().danger } else { cx.theme().secondary_hover }
  }

  #[inline]
  fn active_bg(&self, cx: &mut App) -> Hsla {
    if self.is_close() { cx.theme().danger_active } else { cx.theme().secondary_active }
  }
}
...
impl RenderOnce for WindowControls {
  fn render(self, window: &mut Window, _: &mut App) -> impl IntoElement {
    ...
    h_flex()
      ...
      .child(if window.is_maximized() { ControlIcon::restore() } else { ControlIcon::maximize() })
  ...
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    div().flex_shrink_0().child(
      div()
        ...
        .when(is_macos, |this| this.on_double_click(|_, window, _| window.titlebar_double_click()))
        ...
        .child(WindowControls { on_close_window: self.on_close_window }),
    )
  }
}
