use gpui::{
  ...
  Focusable, InteractiveElement as _, IntoElement, KeyBinding, MouseButton, ParentElement, Pixels,
  Point, Render, RenderOnce, StyleRefinement, Styled, Subscription, Window, anchored, canvas,
...
impl Popover {
  ...
  fn resolved_corner(anchor: Corner, bounds: Bounds<Pixels>) -> Point<Pixels> {
    ...
    }) + Point { x: px(0.), y: -bounds.size.height }
  }
}
...
impl RenderOnce for Popover {
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    let state =
      window.use_keyed_state(self.id.clone(), cx, |_, cx| PopoverState::new(default_open, cx));
