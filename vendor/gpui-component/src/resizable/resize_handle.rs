...
impl<T: 'static, E: 'static + Render> ResizeHandle<T, E> {
  fn new(id: impl Into<ElementId>, axis: Axis) -> Self {
    let id = id.into();
    Self { id: id.clone(), on_drag: None, drag_value: None, placement: None, axis }
  }
...
impl<T: 'static, E: 'static + Render> Element for ResizeHandle<T, E> {
  ...
  fn request_layout(
    ...
    window.with_element_state(id.unwrap(), |state, window| {
      ...
      let bg_color = if state.is_active() { cx.theme().drag_border } else { cx.theme().border };
      ...
      let mut el = div()
        ...
        .when_some(self.on_drag.clone(), |this, on_drag| {
          this.on_drag(self.drag_value.clone().unwrap(), move |_, position, window, cx| {
