...
impl RenderOnce for ResizablePanelGroup {
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    let container = if self.axis.is_horizontal() { h_flex() } else { v_flex() };
    ...
    container
      .id(self.id)
      .size_full()
      .children(self.children.into_iter().enumerate().map(|(ix, mut panel)| {
        panel.panel_ix = ix;
...
impl RenderOnce for ResizablePanel {
  fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    let state = self.state.expect("BUG: The `state` in ResizablePanel should be present.");
    ...
    div()
      ...
      .size_full()
      .relative()
      .when(self.axis.is_vertical(), |this| this.min_h(size_range.start).max_h(size_range.end))
      .when(self.axis.is_horizontal(), |this| this.min_w(size_range.start).max_w(size_range.end))
      ...
      .when_some(self.initial_size, |this, initial_size| {
        ...
        this
          .when(panel_state.size.is_none() && !initial_size.is_zero(), |this| this.flex_none())
