use gpui::{
  ...
  StatefulInteractiveElement as _, StyleRefinement, Styled, Subscription, Window, anchored, canvas,
...
impl ColorPickerState {
  ...
  pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
    ...
    let _subscriptions =
      vec![cx.subscribe_in(&state, window, |this, state, ev: &InputEvent, window, cx| match ev {
        InputEvent::Change => {
  ...
  pub fn set_value(&mut self, value: impl Into<Hsla>, window: &mut Window, cx: &mut Context<Self>) {
...
impl ColorPicker {
  ...
  fn render_item(
    ...
    div()
      ...
      .when(clickable, |this| {
        this
          .hover(|this| this.border_color(color.darken(0.3)).bg(color.lighten(0.1)).shadow_xs())
  ...
  fn render_colors(&self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    v_flex()
      ...
      .child(
        h_flex()
          .gap_1()
          .children(featured_colors.iter().map(|color| self.render_item(*color, true, window, cx))),
      )
      .child(Divider::horizontal())
      .child(v_flex().gap_1().children(color_palettes().iter().map(|sub_colors| {
        h_flex()
          .gap_1()
          .children(sub_colors.iter().rev().map(|color| self.render_item(*color, true, window, cx)))
      })))
              ...
              .child(
                div()
                  ...
                  .when(!display_title.is_empty(), |this| {
                    this
                      .tooltip(move |_, cx| cx.new(|_| Tooltip::new(display_title.clone())).into())
                  }),
              )
