...
use gpui::{
  ...
  SharedString, StatefulInteractiveElement as _, Styled, Window, div, prelude::FluentBuilder as _,
  rems,
};
...
impl RenderOnce for Accordion {
  fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
    ...
    v_flex()
      ...
      .children(self.children.into_iter().enumerate().map(|(ix, accordion)| {
        if accordion.open {
      ...
      .when_some(self.on_toggle_click.filter(|_| !self.disabled), move |this, on_toggle_click| {
        let open_ixs = Rc::clone(&open_ixs);
    ...
    div().flex_1().child(
      v_flex()
        ...
        .when(self.bordered, |this| {
          this.border_1().rounded(cx.theme().radius).border_color(cx.theme().border)
        })
        .text_size(text_size)
        .child(
          h_flex()
            ...
            .when(self.open, |this| {
              this.when(self.bordered, |this| {
                this.text_color(cx.theme().foreground).border_b_1().border_color(cx.theme().border)
              })
            })
            .when(!self.bordered, |this| this.border_b_1().border_color(cx.theme().border))
            .child(
              h_flex()
                .items_center()
                .map(|this| match self.size {
                ...
                .when_some(self.icon, |this, icon| {
                  this.child(icon.with_size(self.size).text_color(cx.theme().muted_foreground))
                })
                .child(self.title),
            )
