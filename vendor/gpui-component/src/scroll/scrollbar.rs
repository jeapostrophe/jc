...
use gpui::{
  ...
  ScrollHandle, ScrollWheelEvent, Size, Style, Timer, UniformListScrollHandle, Window, fill, point,
  px, relative, size,
};
...
impl Element for Scrollbar {
  ...
  fn prepaint(
    ...
    let state = window.use_state(cx, |_, _| ScrollbarState::default()).read(cx).clone();

    let mut states = vec![];
    let mut has_both = self.axis.is_both();
    let scroll_size = self.scroll_size.unwrap_or(self.scroll_handle.content_size());
    ...
    for axis in self.axis.all().into_iter() {
      ...
      let margin_end = if has_both && !is_vertical { WIDTH } else { px(0.) };
      ...
      let bounds = Bounds {
        ...
        size: gpui::Size {
          width: if is_vertical { WIDTH } else { hitbox.size.width },
          height: if is_vertical { hitbox.size.height } else { WIDTH },
        },
      };
    ...
    PrepaintState { hitbox, states, scrollbar_state: state }
  }
  ...
  fn paint(
    ...
    if self.scroll_handle.offset() != scrollbar_state.get().last_scroll_offset {
      scrollbar_state.set(
        scrollbar_state.get().with_last_scroll(self.scroll_handle.offset(), Some(Instant::now())),
      );
      cx.notify(view_id);
    }
        ...
        window.paint_layer(hitbox_bounds, |cx| {
          ...
          cx.paint_quad(PaintQuad {
            ...
            border_widths: if is_vertical {
              Edges { top: px(0.), right: px(0.), bottom: px(0.), left: px(0.) }
            } else {
              Edges { top: px(0.), right: px(0.), bottom: px(0.), left: px(0.) }
            },
          ...
          move |event: &ScrollWheelEvent, phase, _, cx| {
            if phase.bubble() && hitbox_bounds.contains(&event.position) {
              if scroll_handle.offset() != state.get().last_scroll_offset {
                state
                  .set(state.get().with_last_scroll(scroll_handle.offset(), Some(Instant::now())));
                cx.notify(view_id);
              }
            }
          }
