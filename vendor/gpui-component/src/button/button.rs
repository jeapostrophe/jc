...
use crate::{
  ActiveTheme, Colorize as _, Disableable, FocusableExt as _, Icon, IconName, Selectable, Sizable,
  Size, StyleSized, StyledExt, h_flex, spinner::Spinner, tooltip::Tooltip,
};
use gpui::{
  ...
  fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    self
      ...
      .when(self.selected, |this| {
        ...
        this.bg(selected_style.bg).border_color(selected_style.border).text_color(selected_style.fg)
      })
      .when(!self.disabled && !self.selected, |this| {
        this
          ...
          .hover(|this| {
            ...
            this.bg(hover_style.bg).border_color(hover_style.border).text_color(crate::red_400())
          })
          .active(|this| {
            let active_style = style.active(self.outline, cx);
            this.bg(active_style.bg).border_color(active_style.border).text_color(active_style.fg)
          })
      })
      .when(self.disabled, |this| {
      ...
      .when(self.loading && !self.disabled, |this| {
        this
          .bg(normal_style.bg.opacity(0.8))
        ...
        this.tooltip(move |window, cx| {
          Tooltip::new(tooltip.clone())
            .when_some(action.clone(), |this, (action, context)| {
              this.action(action.boxed_clone().as_ref(), context.as_ref().map(|c| c.as_ref()))
            })
            .build(window, cx)
        })
      })
...
impl ButtonVariant {
  fn bg_color(&self, outline: bool, cx: &mut App) -> Hsla {
    ...
    match self {
      ...
      ButtonVariant::Ghost | ButtonVariant::Link | ButtonVariant::Text => cx.theme().transparent,
  ...
  fn disabled(&self, outline: bool, cx: &mut App) -> ButtonVariantStyle {
    let bg = match self {
      ButtonVariant::Link | ButtonVariant::Ghost | ButtonVariant::Text => cx.theme().transparent,
    ...
    let (bg, border) =
      if outline { (cx.theme().transparent, cx.theme().border.opacity(0.5)) } else { (bg, bg) };
