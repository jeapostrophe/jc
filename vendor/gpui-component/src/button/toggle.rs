...
impl RenderOnce for Toggle {
  fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
    ...
    div()
      ...
      .when(checked, |this| this.bg(cx.theme().accent).text_color(cx.theme().accent_foreground))
...
impl RenderOnce for ToggleGroup {
  fn render(self, _: &mut Window, _: &mut App) -> impl IntoElement {
    let disabled = self.disabled;
    let checks = self.items.iter().map(|item| item.checked).collect::<Vec<bool>>();
    ...
    h_flex()
      ...
      .children(self.items.into_iter().enumerate().map({
        |(ix, item)| {
          ...
          item.disabled(disabled).with_size(self.size).with_variant(self.variant).on_click(
            move |_, _, _| {
              state.set(Some(ix));
            },
          )
